...
int main()
{
	...
	App::Init("Week 3 tutorial - LERP plus paths", 2000, 1000);
	...
	// Creating waypoint entity
	Entity ent_waypoint1 = Entity::Create();
	ent_waypoint1.Add<CMeshRenderer>(ent_waypoint1, *mesh_box, *mat_ducky);
	ent_waypoint1.transform.m_scale = glm::vec3(0.005f, 0.005f, 0.005f);
	ent_waypoint1.transform.m_pos = glm::vec3(-1.f, -1.f, 1.f);
	ent_waypoint1.transform.m_rotation = glm::angleAxis(glm::radians(-30.0f), glm::vec3(0.0f, 1.0f, 0.0f));

	// Creating waypoint entity
	Entity ent_waypoint2 = Entity::Create();
	ent_waypoint2.Add<CMeshRenderer>(ent_waypoint2, *mesh_box, *mat_ducky);
	ent_waypoint2.transform.m_scale = glm::vec3(0.005f, 0.005f, 0.005f);
	ent_waypoint2.transform.m_pos = glm::vec3(3.f, -1.0f, 1.f);
	ent_waypoint2.transform.m_rotation = glm::angleAxis(glm::radians(-30.0f), glm::vec3(0.0f, 1.0f, 0.0f));


	// Set up waypoint container
	std::vector<std::unique_ptr<Entity>> points;
	/*std::vector<Entity> points2;
	points2.push_back(ent_waypoint1);
	points2.push_back(ent_waypoint2);*/
	...
	while (!App::IsClosing() && !Input::GetKeyDown(GLFW_KEY_ESCAPE))
	{
		...
		ent_waypoint1.Get<CMeshRenderer>().Draw();
		ent_waypoint2.Get<CMeshRenderer>().Draw();
